...
import { indentLines, processInputs, convertBlocksPart } from "./scratch-conversion-helper.mjs";
...
export default class ScratchConversionControl {
    ...
    * @returns {string}
    */
   convertControlBlock(target, currentBlockId) {
      ...
      switch (opcode) {
         case "control_forever": {
            // Forever loop
            let SUBSTACK = convertBlocksPart(target, currentBlockId, currentBlock.inputs.SUBSTACK[1]).script;
         ...
         case "control_if": {
            ...
            let CONDITION = convertBlocksPart(target, currentBlockId, currentBlock.inputs.CONDITION[1]).script;
         ...
         case "control_if_else": {
            ...
            let SUBSTACK = convertBlocksPart(target, currentBlockId, currentBlock.inputs.SUBSTACK[1]).script;
            SUBSTACK = SUBSTACK.substring(0, SUBSTACK.length - 1);
            let SUBSTACK2 = convertBlocksPart(target, currentBlockId, currentBlock.inputs.SUBSTACK2[1]).script;
